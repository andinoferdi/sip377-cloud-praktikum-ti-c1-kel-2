...
export async function GET() {
  ...
  return buildNotImplementedResponse("GET /api/pos/sales");
}

export async function POST() {
  const guard = await requirePermission("sales:create");
  if (isGuardBlocked(guard)) {
    return guard.response;
  }

  return buildNotImplementedResponse("POST /api/pos/sales");
}
